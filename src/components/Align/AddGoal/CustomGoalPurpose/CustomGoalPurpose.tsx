...
import AddClarifyingStatements from "../AddGoalPurpose/AddClarifyingStatements";
...
import {updateCustomGoalPurpose} from "../../../../actions/align/updateCustomGoalPurpose";
import {describe} from "node:test";

const CustomGoalPurpose=({
  closePopup,
  open,
  goal,
  setGoal,
  ...
  const [startDate,setStartDate]=useState<any>(goal?.startDate? dayjs(goal?.startDate):null);
  ...
  const [goalType,setGoalType]=useState<any>(null);
  ...
  useEffect(() => {
    const fetchEmployeeAddGoalQuestion=async () => {
      const response=await fetchAlignQuestions({programId: user?.activeProgramId,type: currentUserRole===MANAGER_VIEW_STATE.LP? ALIGN_QUESTION_TYPES.ADD_GOAL_EMPLOYEE_QUESION:ALIGN_QUESTION_TYPES.ADD_GOAL_MANAGER_QUESTION});
  ...
  useEffect(() => {
    ...
    if(currentUserRole===MANAGER_VIEW_STATE.LP) {
      if(goal?.addGoalEmployeeQuestion) {
        setAddGoalEmployeeQuestion(goal?.addGoalEmployeeQuestion)
      } else {
        setAddGoalEmployeeQuestion(null);

      }
    } else {
      if(goal?.addGoalManagerQuestion) {
        setAddGoalEmployeeQuestion(goal?.addGoalManagerQuestion)
      } else {
        setAddGoalEmployeeQuestion(null);
  ...
  console.log("addGoalEmployeeQuestion",goal,addGoalEmployeeQuestion);
  ...
  const addNewQuestion=() => {
    console.log(isAnswer,isQuestion,"isQuestion");
    const temp=[...addGoalEmployeeQuestion];
    const newStatement={
      question: isQuestion,
      regular: true, // Assuming regular is always true
      mandatory: false, // Assuming mandatory is always true
      type: null, // Assuming type is always "PURPOSE"
      tags: [], // You might want to set tags based on your requirements
      order: temp.length+1, // Setting the order based on the current length of the array
      answer: isAnswer,
      selectedTags: [] // You might want to set selectedTags based on your requirements
    };
    temp.push(newStatement);
    setAddGoalEmployeeQuestion(temp);
    setStatement("");
    setStChange(!stChange);
  ...
  const saveAndUpdatePurpose=async () => {
    try {
      ...
      const response=await updateCustomGoalPurpose({
        userId,
        programId,
        goalId: goal.id,
  ...
  console.log(addGoalEmployeeQuestion,"adityaddalignquestion");
          ...
          <DialogContent sx={{padding: "0 0 0px 0",marginBottom: "20px"}}>
            ...
            <Stack flexDirection="row" className="add_goal_details_flex">
              <Box className="popup_left_box">
                ...
                <Typography className="popup_txtfld_lbl" sx={{marginTop: "20px"}}>
                  Goal Type&#x2A;
                </Typography>
                <RadioGroup
                  row
                  ...
                  name="goal-category"
                  sx={{marginLeft: "10px",alignContent: "center"}}
                  value={goalType}
                  onChange={(e) => setGoalType(e.target.value)}
                >{program?.goalTypes?.map((type: string,index: number) => {
                  return (
                    <FormControlLabel
                      value={type}
                      control={<Radio />}
                        ...
                        return (
                          <>
                            ...
                            >
                              ...
                              />
                            </Box>:<div className="gl-modify-txtbox-clr txtbox-clr2">
...
export default CustomGoalPurpose;