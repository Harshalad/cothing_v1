...
const UpcomingEvents = ({ upcomingScheduledEvents }:any) => {
   const router = useRouter();
   const currentUserRole = useSelector(
     //@ts-ignore
     (state) => state?.auth?.managerToggleView
   );
   const handleViewDeatilsClick =(event:any)=>{
    router.push({
      pathname: "/events",
      query: {
        id:  event?.id 
  ...
  return (
    <>
      <Stack className="active_assmnts_card_flx slct_evnt_slot_contr">
        {upcomingScheduledEvents?.map((event:any, index:any)=>{
          return (
            <Box className="active_assmnts_card" key={index}>
              ...
              <Box className="analyze_cta">
                <Button className="standard_cta" onClick={()=> handleViewDeatilsClick(event)}>View Details</Button>
              </Box>
            </Box>
          );
        })}
        {/* <Box className="active_assmnts_card">
          ...
          <Box className="analyze_cta">
            <Button className="standard_cta">View Details</Button>
          </Box>
        </Box> */}
      </Stack>
    </>
